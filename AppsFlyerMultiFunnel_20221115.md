...
WITH
  ...
  clicks_retargeting AS (
    SELECT
        '2022-04-01 19:03:02 UTC' AS event_time,
        'click' AS event_name,
  ...
  raw_data AS
    ...
    (SELECT
        ...
        CASE
            ...
            WHEN LAG(channel) OVER (PARTITION BY appsflyer_id, customer_user_id, is_retarg ORDER BY touch_time ASC ) = channel
                -- 7 - say, it is our avg. time per session; if diff between time more than 7 minutes = new channel (direct/none)
                AND DATE_DIFF(touch_time, LAG(touch_time) OVER (PARTITION BY appsflyer_id ORDER BY touch_time ASC), MINUTE) >= 7
